namespace BuildingsIterator
{
    ...
    public class ScramblingGenerator
    {
        ...
        public ChainPicksWithCharacteristics Generate(Alphabet alphabet, int len, List<LinkedCharacteristic> charact)
        {
            ...
            {
                Chain chain = this.GenerateChain(i);
                List<double> characteristics = this.CalculateCharacteristics(chain);
                try
                {
        ...
        {
            var characteristics = new List<double>(this.characteristic.Count);
            foreach (LinkedCharacteristic calculator in this.characteristic)
            {
        ...
        private Chain GenerateChain(int i)
        {
            ...
            {
                var element = (int)(temp / Math.Pow(this.alphabet.Cardinality, j));
                chain.Add(this.alphabet[element], index);
                temp = (int)(temp % Math.Pow(this.alphabet.Cardinality, j));
                index++;
            }
