...
namespace TestChainAnalysis.Classes.TheoryOfGraphs
{
    ...
    ///</summary>
    [TestFixture]
    public class TestLShortestOpenLoopPathBuilder
    {
        ...
        public void TestBuild()
        {
            ...
            TestGraph.Add(new Branch(1, 3), 1);
            TestGraph.Add(new Branch(3, 2), 2);
            TestGraph.Add(new Branch(4, 1), 3);
            TestGraph.Add(new Branch(4, 5), 4);
            TestGraph.Add(new Branch(5, 6), 5);

            Assert.AreEqual(5, LSOLP.Count);
            foreach (Branch branch in TestGraph.Branches)
            {
                Assert.IsTrue(LSOLP.Contains(branch));
            }
        }
    }
}