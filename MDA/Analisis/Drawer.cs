...
namespace MDA.Analisis
{
    public class Drawer
    {
        ...
        public void DrawZGC(DataGridView d, ZedGraphControl z, ArrayList data1,ArrayList data2, double MaxX, double MaxY,string G,string x,string y)
        {
            d.Rows.Clear();
        ...
        public void DrawGraphs(Form1 f, DisplayData dd, int number)
        {
            switch(number)
            {
                    case 1: DrawZGC(f.dataGridView1,f.zedGraphControl1, dd.Id_N,null, dd.Id_N.Count, dd.GreatOccur,"Graph nj(Id)","Id","nj");
                    break;
                    case 2: DrawZGC(f.dataGridView2, f.zedGraphControl2, dd.Rank_FreqP, dd.Rank_FreqT, Math.Max(dd.Rank_FreqP.Count,dd.Rank_FreqT.Count),dd.GreatFrequency, "Graph Freq(Rank)", "Rank", "Freq");
                    break;
                    case 3: DrawZGC(f.dataGridView3, f.zedGraphControl3, dd.LogRank_LogNP, dd.LogRank_LogNT, Math.Log(Math.Max(dd.LogRank_LogNP.Count, dd.LogRank_LogNT.Count), 2), Math.Log(dd.GreatOccur, 2), "Graph Log(nj)(Log(Rank))", "Log(Rank)", "Log(nj)");
                    break;
                    case 4: DrawZGC(f.dataGridView4, f.zedGraphControl4, dd.LogRank_LogGamut, null, Math.Log(dd.LogRank_LogGamut.Count, 2), dd.GreatLogGamut, "Graph Log(Gr)(Log(Rank))", "Log(Rank)", "Log(Gr)");
                    break;
                    case 5: DrawZGC(f.dataGridView5, f.zedGraphControl5, dd.Rank_Remoteness, null, dd.Rank_Remoteness.Count, dd.GreatRemoteness, "Graph gr(Rank)", "Rank", "gr");
                    break;
            }
        ...
        public void Draw (Form1 f,DisplayData dd)
    {
            ...
            f.textBox7.Text = Convert.ToString(dd.DiffRFreq.GetD());
            f.textBox6.Text = Convert.ToString(dd.DiffRFreq.GetSqHi());
            f.textBox5.Text = Convert.ToString(dd.DiffRFreq.GetOnonP());
            f.textBox4.Text = Convert.ToString(dd.DiffRFreq.GetnonP());
            f.textBox3.Text = Convert.ToString(dd.TLCapacity);
            f.textBox2.Text = Convert.ToString(dd.PLCapacity);
            f.textBox10.Text = Convert.ToString(dd.DiffV.GetpdV());
            f.textBox11.Text = Convert.ToString(dd.DiffV.GetdV());
